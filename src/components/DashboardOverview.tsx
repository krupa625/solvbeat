...
import { Shield, Zap, Search, Server, Scale, Code, AlertTriangle, CheckCircle2, Info, ArrowUpRight, Award, ShieldAlert, Sparkles, TrendingUp } from "lucide-react";
import { ScanResult, CategoryScore } from "../types";
...
export default function DashboardOverview({ scanData }: DashboardOverviewProps) {
  ...
  const getHeatmapColor = (score: number) => {
    if (score >= 80) return "bg-rose-500/20 text-rose-400 border-rose-500/30";
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div className="text-left">
            ...
            <h2 className="text-4xl font-display font-medium text-slate-50 tracking-tight">
              Website Digital Health Overview
            </h2>
            <p className="text-slate-400 mt-2 max-w-xl text-sm leading-relaxed">
              Detailed technical diagnostic scores for <span className="text-sky-400 font-mono text-xs">{scanData.url}</span> based on raw server metrics and AI analysis.
            </p>
          </div>

          {/* Tab buttons */}
          <div className="flex p-1 bg-zinc-950 border border-zinc-900 rounded-lg self-start">
            <button
        ...
        {activeTab === 'overview' && (
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 bg-zinc-950/45 border border-zinc-900 rounded-2xl p-8 backdrop-blur-xl items-center relative overflow-hidden">
              ...
              <div className="md:col-span-4 flex flex-col items-center justify-center text-center">
                <div className="relative w-44 h-44 flex items-center justify-center">
                  ...
                  <div className="absolute flex flex-col items-center">
                    <span className="text-5xl font-display font-semibold text-slate-50 tracking-tighter">
                      {scanData.overallScore}
                    </span>
                    <span className="text-[10px] font-mono text-purple-400 tracking-wider uppercase mt-1">
                      GLOBAL RATIO
                    </span>
                  </div>
                </div>
              ...
              <div className="md:col-span-8 space-y-6 text-left">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-display text-slate-100 font-medium">Excellent Cyber Standings</h3>
                    <p className="text-xs text-slate-400 mt-0.5">Overall compliance ratings are in the top tier of international standard targets.</p>
                  </div>
                </div>

                {/* Grid stats */}
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-4 border-t border-zinc-900/80 font-mono">
                  <div className="p-3 bg-zinc-950/80 border border-zinc-900 rounded-lg">
                    <div className="text-[10px] text-slate-400">SERVER IP</div>
                    <div className="text-xs text-sky-400 mt-1 font-semibold overflow-ellipsis overflow-hidden">{scanData.ipAddress}</div>
                  </div>
                  <div className="p-3 bg-zinc-950/80 border border-zinc-900 rounded-lg">
                    <div className="text-[10px] text-slate-400">LATENCY DURATION</div>
                    <div className="text-xs text-purple-400 mt-1 font-semibold">{scanData.scanTimeMs} ms</div>
                  </div>
                  <div className="p-3 bg-zinc-950/80 border border-zinc-900 rounded-lg col-span-2 sm:col-span-1">
                    <div className="text-[10px] text-slate-400">SECURITY CHECKS</div>
                    <div className="text-xs text-pink-400 mt-1 font-semibold">82 Audits Verified</div>
                  </div>
                </div>
              </div>
            </div>

            {/* Grid of Six Category Cards */}
              ...
              {Object.entries(scanData.categories).map(([key, cat]) => {
                ...
                return (
                  <div
                    ...
                    className="group relative bg-zinc-950/40 border border-zinc-900 rounded-2xl p-6 transition-all duration-300 hover:border-zinc-800 backdrop-blur-xl flex flex-col justify-between overflow-hidden"
                  >
                    ...
                    <div>
                      ...
                      <div className="flex justify-between items-start mb-5">
                        <div className={`p-2.5 rounded-xl border ${config.color} flex items-center justify-center transition-transform group-hover:scale-105 duration-300`}>
                          <IconComponent className="w-5 h-5" />
                        </div>
                        <span className={`px-2 py-0.5 rounded text-[9px] font-mono border uppercase tracking-wider ${getStatusColor(cat.status)}`}>
                          {cat.status}
                        </span>
                      </div>
                      ...
                      <div className="flex items-baseline gap-2">
                        <span className="text-3xl font-display font-medium text-slate-50 tracking-tight">
                          {cat.score}
                        </span>
                        <span className="text-xs text-slate-400 font-mono">/100</span>
                        <span className="text-xs text-purple-400 font-semibold font-mono ml-1">Grade {cat.grade}</span>
                      </div>

                      <h3 className="text-sm font-display text-slate-100 font-medium mt-1">
                        {cat.name}
                      </h3>
                      <p className="text-xs text-slate-400 mt-2 leading-relaxed">
                        Evaluated {cat.checkedCount} core check points with {cat.passedCount} successes.
                      </p>
                    </div>

                    {/* Sparkline Visual at footer */}
                    <div className="mt-6 pt-4 border-t border-zinc-900/60 flex items-center justify-between">
                      <span className="text-[9px] font-mono text-slate-500 tracking-widest">{config.tag}</span>
        ...
        {activeTab === 'threat-map' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Heatmap Info Column */}
            <div className="lg:col-span-1 bg-zinc-950/40 border border-zinc-900 rounded-2xl p-6 backdrop-blur-xl flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 text-rose-400 font-mono text-xs mb-3">
                  <ShieldAlert className="w-4 h-4" />
                  REAL-TIME RISK COVERAGE
                </div>
                <h3 className="text-xl font-display font-medium text-slate-100">Vulnerability Map</h3>
                <p className="text-xs text-slate-400 mt-2 leading-relaxed">
                  Heatmap matrix illustrating simulated exploit risks and config flaws mapped across Solvbeat audit directories.
                </p>

                {/* Score indicators list */}
                <div className="space-y-3 mt-6 font-mono text-xs">
                  <div className="flex justify-between items-center p-2.5 bg-zinc-950/80 border border-zinc-900 rounded-lg">
              ...
              <div className="pt-6 border-t border-zinc-900 mt-8 flex items-center gap-3">
                ...
                <div className="text-left font-mono">
                  <div className="text-[10px] text-slate-400">GLOBAL COMPLIANCE</div>
                  <div className="text-xs text-slate-200">92.4% SECURE RATIO</div>
                </div>
              </div>
            </div>

            {/* Grid Heatmap Column */}
            <div className="lg:col-span-2 bg-zinc-950/40 border border-zinc-900 rounded-2xl p-6 backdrop-blur-xl">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {threatMatrix.map((item, index) => (
                  <div
                    key={index}
                    className={`border rounded-xl p-4 flex flex-col justify-between transition-transform hover:scale-[1.02] duration-200 ${getHeatmapColor(item.score)}`}
                  >
                    <div>
                      <div className="flex justify-between items-start">
                        <span className="text-[9px] font-mono uppercase tracking-widest">{item.group}</span>
                        <span className="text-[10px] font-mono font-semibold">{item.risk}</span>
                      </div>
                      <h4 className="text-sm font-display font-medium text-slate-200 mt-2">{item.name}</h4>
                    </div>
        ...
        {activeTab === 'remediation' && (
          <div className="bg-zinc-950/40 border border-zinc-900 rounded-2xl p-8 backdrop-blur-xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-80 h-80 bg-purple-500/5 rounded-full blur-3xl pointer-events-none" />
            ...
            <h3 className="text-2xl font-display font-medium text-slate-100">
              AI-Generated Digital Threat & Architectural Report
            </h3>
            
            {/* The multi-paragraph output formatted elegantly */}
            <div className="mt-6 text-sm text-slate-300 leading-relaxed space-y-6 text-left max-w-4xl">
              {scanData.aiAnalysis.split("\n\n").map((para, pidx) => (
                <p key={pidx} className="first-letter:text-xl first-letter:font-semibold first-letter:text-sky-400">
                  {para}
                </p>
              ))}
            </div>

            <div className="mt-8 pt-6 border-t border-zinc-900/80 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="w-2 h-2 bg-emerald-400 rounded-full animate-ping" />
                <span className="text-xs font-mono text-slate-400">AI MODEL VERIFIED: GEMINI-3.5-FLASH</span>
              </div>
              <span className="text-xs text-slate-500 font-mono">REPORT IDENTIFIER: SB-SCAN-{Math.floor(100000 + Math.random() * 900000)}</span>
            </div>
          </div>
        )}
